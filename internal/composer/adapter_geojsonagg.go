...
func (a *GeoJSONV2Adapter) MergeWithQuery(_ context.Context, q QueryParams, parts [][]byte) ([]byte, error) {
	req := geojsonagg.Request{
		...
		Shards: make([]geojsonagg.ShardPage, 0, len(parts)),
	}
	...
	for i, p := range parts {
		...
		req.Shards = append(req.Shards, geojsonagg.ShardPage{
			Meta:     geojsonagg.ShardMeta{FromCache: false, ID: fmt.Sprintf("part-%d", i)},
			Features: feats,
		})
	}
